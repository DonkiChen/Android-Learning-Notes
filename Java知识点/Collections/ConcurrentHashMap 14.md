...
继承自 `ConcurrentHashMap.Node`, 只是一个代理类, 不保存节点的key, value, 只是用作维护树节点, 树根节点, 锁状态等. 是红黑树根节点的的桥接类, 存在哈希表中, 即哈希表中是红黑树结构的, 存的都是TreeBin类型
...
1. 如果先有线程读, 那么不允许后来的进程写
2. 如果先有线程写, 允许以节点的next指针, 即**允许作为链表读**, 因为**写的过程中红黑树结构可能会改变, 但是链表的顺序不会改变**
